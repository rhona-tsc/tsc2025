...
const ElectricDrumsToggle = ({ lineup, lineupIndex, setLineups }) => {
  ...
  return (
    <div className="grid grid-cols-3 items-center gap-x-2 gap-y-3 mb-4">
      ...
      <div className="col-span-1 flex justify-center items-center gap-2 h-full">
        <div
          className={`toggle ${lineup.eDrums ? "on" : "off"}`}
          onClick={() => updateLineup("eDrums", !lineup.eDrums)}
          style={{
            cursor: "pointer",
            width: "60px",
            height: "30px",
            borderRadius: "15px",
            backgroundColor: lineup.eDrums ? "#ff6667" : "#ccc",
            position: "relative",
          }}
        >
          <div
        ...
        <span>{lineup.eDrums ? "Yes" : "No"}</span>
      </div>
...
export default ElectricDrumsToggle;