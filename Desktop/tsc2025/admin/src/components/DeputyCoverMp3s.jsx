...
const DeputyCoverMp3s = ({ mp3s = [], setMp3s, isUploading, setIsUploading }) => {
  const onDrop = async (acceptedFiles) => {
    ...
    setMp3s((prev) => [...(Array.isArray(prev) ? prev : []), ...uploadedMp3s]);
  ...
  const removeMp3 = (id) => {
    setMp3s((prev) =>
      Array.isArray(prev) ? prev.filter((mp3) => mp3.id !== id) : []
    );
  };
        ...
        >
          ...
          <p className="text-sm text-gray-500">
            {isUploading ? (
              ...
              "Drag & drop your cover MP3s here, or click to browse"
            )}
          </p>
        </div>
...
export default DeputyCoverMp3s;